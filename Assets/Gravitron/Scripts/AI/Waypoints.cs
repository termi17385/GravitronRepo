...
public class Waypoints : MonoBehaviour
{
	...
	private void Update()
	{
		if(ship != null) shipLanded = ship.GetComponent<SpaceShip>().hasLanded;
	...
	private void OnTriggerStay2D(Collider2D other)
	{
		if(other.gameObject.CompareTag("Player"))
		{
			...
			if(spaceShip.hasLanded)
			{
				Debug.Log("Has Landed");
				ship = spaceShip.transform;
				shipLanded = true;
			}
		}
	}
	...
	private void PickUpUnits()
	{
		...
		{
			currentSpaceMen.currentWaypoint = ship.GetComponent<SpaceShip>().dockingPort;
			...
			if(dist <= 0.2f)
			{
				...
				ship.GetComponent<SpaceShip>().SpaceMenPickedUp();
